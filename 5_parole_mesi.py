...
def tokenizza_file(percorso_file):
    with open(percorso_file, "r", encoding="utf-8") as file:
        ...
        tokens_validi = [token.text for token in doc 
                         ...
                         and token.lemma_ not in {"è", "più", "pr", "\n", "o", "il", "a il", "di il", "perchè", "in il", "coty", "iv/"}
...
for idx, mese in enumerate(mesi_ordine):
    if mese in dizionario:  # dizionario{mese:relazioni}, non in dizionario gennaio, febbraio, agosto
        ...
        fig.add_trace(go.Scatter(x=[x_coords[idx]], y=[y_coords[idx]] ,mode='markers+text', marker=dict(size=50, color=colori[idx % len(colori)]), text=mese,textposition="bottom center", hovertemplate=f"<b>{mese}</b><br>{hover_text}<extra></extra>", name=mese))
    else:  # mesi senza relazioni: gennaio, febbraio, agosto
        fig.add_trace(go.Scatter(x=[x_coords[idx]], y=[y_coords[idx]], mode='markers+text',marker=dict(size=40, color='lightgrey', line=dict(color='black', width=1)), text=mese, textposition="bottom center", hovertemplate=f"<b>{mese}</b><br>No Data<extra></extra>", name=mese))

# 1947, anno a cui tutti i mesi fanno riferimento 
fig.add_trace(go.Scatter(x=[0], y=[0], mode='text', text=["1947"], textfont=dict(size=50, color="black"),showlegend=False, hoverinfo="skip"))

fig.update_layout(title="1947: Lessico dell'assembela costituente rapppresentativo per ogni mese di discussione", xaxis=dict(showticklabels=False, zeroline=False), yaxis=dict(showticklabels=False, zeroline=False), plot_bgcolor="white", height=800, width=800)

output_html = "parole_significative_mesi_1947.htmll"
...
# Tempo totale di esecuzione ultima run: 700.44 secondi = 11, 6 minuti