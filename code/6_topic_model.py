...
pickle.dump(corpus_tfidf, open('modello_nuovo/corpus.pkl', 'wb'))
dictionary.save('modello_nuovo/dictionary.gensim')
...
ldamodel.save('modello_nuovo/nuovo_topic8.gensim')

topics = ldamodel.print_topics(num_words=7)
for topic in topics:
    print(topic)

dictionary_loaded = gensim.corpora.Dictionary.load('modello_nuovo/dictionary.gensim')
corpus_tfidf = pickle.load(open('modello_nuovo/corpus.pkl', 'rb'))

lda = gensim.models.ldamodel.LdaModel.load('modello_nuovo/nuovo_topic8.gensim')
lda_display = gensimvis.prepare(lda, corpus_tfidf, dictionary_loaded, sort_topics=False)

pyLDAvis.save_html(lda_display, 'lda_visualization_nuova.html')
